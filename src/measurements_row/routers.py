...
@router.get("/measurementrows/{sensor}", response_model=schemas.MeasurementRow)
def read_measurement_row(sensor: int, db: Session = Depends(get_db)):
    db_measurement_row = crud.get_measurement_row(db, sensor)
...
def update_existing_measurement_row(sensor: int, measurement_row: schemas.MeasurementRowCreateUpdate, db: Session = Depends(get_db)):
